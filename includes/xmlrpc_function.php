...
function add_xml_rpc_methods( $methods ) {
    ...
    $methods['pltm.addMatch'] = 'add_match';
...
function add_match($params){
    ...
    $wp_tournament_id        = tournamentCPT::get_tournament_id_by($challonge_tournament_id);

    //if return false then no tournament with that challonge tournament id was found return error!
    if(!$wp_tournament_id)
        return new IXR_Error( 500, __( "No tournament with that challonge tournament id (".$challonge_tournament_id.") was found" ));
    ...
    $p2p_result = p2p_type('tournament_matches')->connect($wp_tournament_id, $match_id, $connection_meta);
